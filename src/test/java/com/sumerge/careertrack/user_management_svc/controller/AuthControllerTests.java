...
public class AuthControllerTests {
    ...
    //TODO 01: Ask others if we have to add DoesNotExist Exception on Department and Title
    @Test
    ...
    public void register_EmailAlreadyExists_ThrowsAlreadyExistsException() throws Exception {
        when(authService.register(this.registerRequest)).thenThrow(new AlreadyExistsException(AlreadyExistsException.APP_USER_EMAIL,this.registerRequest.getEmail()));
        ...
        response.andExpect(status().isBadRequest())
                ...
                .andExpect(content().string(String.format(AlreadyExistsException.APP_USER_EMAIL,this.registerRequest.getEmail())));
    }

    @Test
    public void register_WrongDepartment_ThrowsDoesNotExistException() throws Exception {
        when(authService.register(this.registerRequest)).thenThrow(new DoesNotExistException(DoesNotExistException.DEPARTMENT,this.testUser.getDepartment().getName()));
        ...
        response.andExpect(status().isBadRequest())
                ...
                .andExpect(content().string(String.format(DoesNotExistException.DEPARTMENT,this.testUser.getDepartment().getName())));
    }
    @Test
    public void register_WrongTitle_ThrowsDoesNotExistException() throws Exception {
        when(authService.register(this.registerRequest)).thenThrow(new DoesNotExistException(DoesNotExistException.TITLE,this.testUser.getTitle().getName(),this.testUser.getDepartment().getName()));
        ...
        response.andExpect(status().isBadRequest())
                .andExpect(result -> assertTrue(result.getResolvedException() instanceof DoesNotExistException))
                .andExpect(content().string(String.format(DoesNotExistException.TITLE,this.testUser.getTitle().getName(),this.testUser.getDepartment().getName())));
    }
    @Test
    public void register_WrongManager_ThrowsDoesNotExistException() throws Exception {
        when(authService.register(this.registerRequest)).thenThrow(new DoesNotExistException(DoesNotExistException.APP_USER_ID,this.testUser.getId()));
        ...
        response.andExpect(status().isBadRequest())
                .andExpect(result -> assertTrue(result.getResolvedException() instanceof DoesNotExistException))
                .andExpect(content().string(String.format(DoesNotExistException.APP_USER_ID,this.testUser.getId())));
    }
    @Test
    ...
    /*TODO 02: Why do we have DoesNotExist As well as InvalidCredentials ??? Check login in AuthService*/
    @Test
    public void login_WrongEmail_ThrowsDoesNotExist() throws Exception {

        when(authService.login(authRequest)).thenThrow(new DoesNotExistException(String.format(DoesNotExistException.APP_USER_EMAIL,authRequest.getEmail())));
        ...
        response.andExpect(status().isBadRequest())
                .andExpect(result -> assertTrue(result.getResolvedException() instanceof DoesNotExistException))
                .andExpect(content().string(String.format(DoesNotExistException.APP_USER_EMAIL,authRequest.getEmail())));
    }
    ...
    public void login_InvalidCredentials_ThrowsInvalidCredentials() throws Exception {
        ...
        when(authService.login(authRequest)).thenThrow(new InvalidCredentialsException(InvalidCredentialsException.DEFAULT));
